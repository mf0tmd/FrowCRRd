...
const getRunnerExecutableName = () => 'frowcrrd_runner.exe';
...
ipcMain.handle('simulation:run', async (_event, payload) => {
  ...
  try {
    ...
    const cwdCandidates = [
      // Packaged app (current layout): resources/FrowCRRd/Configs/config.json
      path.join(process.resourcesPath, 'FrowCRRd'),
      // Packaged app (legacy layouts)
...
app.whenReady().then(() => {
  if (!isWindows) {
    dialog.showErrorBox(
      ...
      'This build of FrowCRRD supports Windows only.'
    );
    app.quit();
    return;
  }
